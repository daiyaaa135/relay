...
export type BrowseDevice = {
  ...
  year?: number | null;
  size?: string | null;
  /** Populated for Video Games: the console this game belongs to (e.g. "PlayStation 5"). */
  console?: string | null;
};
...
/** Ordered list of Video Games consoles as they should appear in the browse screen. */
...
/** Load devices from CSV; returns flat list. */
async function loadFromCsv(
  category: string,
  csvPath: string
): Promise<BrowseDevice[]> {
  ...
  // For video-games.csv: MODEL = game title; for older CSVs: model/console column = the device name
...
/** Sort brands: Apple first, then alphabetical. */
...
/** Normalize brand names so casing variants (e.g. "ASUS" vs "Asus") collapse into one bucket. */
function canonicalBrandName(brand: string): string {
  const raw = (brand || '').trim();
  if (!raw) return 'Other';
  const lower = raw.toLowerCase();

  // Explicit mappings for known brands where we care about exact casing,
  // including multi-word brands.
  ...
  // Default: title-case first character and keep remaining as-is.
...
function sortDevicesByListingCount(
  devices: BrowseDevice[],
  listingCounts: Map<string, number>
): BrowseDevice[] {
  if (!devices.length || listingCounts.size === 0) return devices;
  return [...devices].sort((a, b) => {
    const keyA = makeListingKey(a.brand, a.model);
    const keyB = makeListingKey(b.brand, b.model);
    const countA = listingCounts.get(keyA) ?? 0;
    const countB = listingCounts.get(keyB) ?? 0;
...
export async function GET(
  ...
  // Precompute listing counts per (brand, model) from live gadgets so browse
  // rows can be ordered by popularity (most listings first).
  ...
  // Video Games: group by console, ordered per VIDEO_GAME_CONSOLE_ORDER
  if (category === 'Video Games') {
    ...
    const brands = orderedKeys.map((consoleName) => ({
      brand: consoleName,
      devices: sortDevicesByListingCount(byConsole.get(consoleName) ?? [], listingCounts),
    }));
    return Response.json({ category, brands } satisfies BrowseResponse);
  }
  ...
  const brands = sortedBrandNames.map((brand) => ({
    brand,
