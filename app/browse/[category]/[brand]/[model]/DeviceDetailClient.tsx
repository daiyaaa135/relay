...
import type { DeviceListingsResponse, DeviceListing } from '@/lib/device-listings';
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1.5 w-[76px] flex-shrink-0">
        <div className="relative size-[76px] rounded-xl overflow-hidden bg-[#FBFBFB] dark:bg-relay-surface-dark">
          {listing.image_url ? (
            <Image src={listing.image_url} alt="" fill className="object-cover" sizes="76px" />
          ) : (
...
function DeviceDetailContent({ initialData }: { initialData?: DeviceListingsResponse | null }) {
  ...
  const skipInitialFetch = initialData != null;

  const [listings, setListings] = useState<DeviceListing[]>(initialData?.listings ?? []);
  ...
  useEffect(() => {
    ...
    if (skipInitialFetch) return; // Server pre-fetched page 1
    setOffset(0);
    fetchListings(0);
  }, [brand, model, category, fetchListings, skipInitialFetch]);
  ...
  return (
    <div className="flex-1 min-h-0 bg-relay-surface dark:bg-relay-surface-dark flex flex-col">
      ...
      <main className="pb-20 bg-[#F2F2F2] dark:bg-relay-surface-dark">
          ...
          {!loading && listings.length === 0 && (
            <div className="py-16 text-center flex flex-col items-center">
              <div className="size-20 flex items-center justify-center mb-6">
                <Image src="/no-gear-found.png" alt="" width={80} height={80} className="object-contain" />
              </div>
...
export function DeviceDetailClient({ initialData }: { initialData?: DeviceListingsResponse | null }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <DeviceDetailContent initialData={initialData} />
    </Suspense>
  );
}
