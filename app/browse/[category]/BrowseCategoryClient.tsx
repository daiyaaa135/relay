...
import type { BrowseResponse } from '@/lib/browse';
...
export function BrowseCategoryClient({ initialData }: { initialData?: BrowseResponse | null }) {
  const router = useRouter();
  const params = useParams();
  const rawParam = (params?.category as string) || '';
  const category = rawParam ? decodeURIComponent(rawParam).trim() : '';

  const [data, setData] = useState<BrowseResponse | null>(initialData ?? null);
  ...
  // True only on the first render if server pre-supplied data — skip the initial client fetch
  ...
  useEffect(() => {
    ...
    if (skipInitialFetch) {
      // Server already fetched the data — no client waterfall needed
      return;
    }
    setLoading(true);
    fetchData().finally(() => setLoading(false));
  }, [fetchData, category, isValid, skipInitialFetch]);
