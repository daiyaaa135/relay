...
export default function FollowingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) {
        setUsers(
          (data ?? [])
            .map((r) => r.following as unknown as UserRow)
            .filter(Boolean)
        );
        setLoading(false);
      }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      >
        <h1 className="font-semibold text-relay-text dark:text-relay-text-dark text-base">Following</h1>
      </PageHeader>
        ...
        <div className="pb-8">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center pt-24 px-8 text-center">
              <span className="material-symbols-outlined text-4xl text-relay-muted mb-3">person_search</span>
              <p className="text-relay-text dark:text-relay-text-dark font-semibold">Not following anyone yet</p>
              <p className="text-relay-muted dark:text-relay-muted-light text-sm mt-1">Find people to follow on the Explore page.</p>
            </div>
          ) : (
