-- ReLay Circular Tech Marketplace Schema
-- Credit-based economy, local pickup only, distance-based matching

-- Enable UUID extension (usually already enabled in Supabase)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Enable earthdistance for geo queries (distance-based matching)
CREATE EXTENSION IF NOT EXISTS "cube";
CREATE EXTENSION IF NOT EXISTS "earthdistance";

-- =============================================================================
-- PROFILES (extends auth.users - ReLay+ members with credits)
-- =============================================================================
CREATE TABLE profiles (
  ...
  city TEXT,
  state TEXT,
  -- For distance search: max_pickup_miles (null = use default)
...
CREATE TABLE gadgets (
  ...
  -- Pickup location (inherits from profile or override)
  latitude DOUBLE PRECISION,
  longitude DOUBLE PRECISION,
  city TEXT,
  state TEXT,
...
CREATE INDEX idx_gadgets_profile ON gadgets(profile_id);
CREATE INDEX idx_gadgets_status ON gadgets(status) WHERE status = 'available';
CREATE INDEX idx_gadgets_category ON gadgets(category);
CREATE INDEX idx_gadgets_location ON gadgets(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
CREATE INDEX idx_gadgets_created ON gadgets(created_at DESC);

-- =============================================================================
-- TRANSACTIONS (credit ledger - all credit movements)
-- =============================================================================
CREATE TABLE transactions (
  ...
  type TEXT NOT NULL CHECK (type IN (
    'listing_credit',   -- credits earned when listing sold
    'swap_debit',       -- credits spent on a swap
    'swap_credit',      -- credits received from a swap
    'monthly_fee',      -- membership fee
    'referral_bonus',   -- referral reward
    'system_adjustment' -- admin/support
  )),
  description TEXT,
  reference_id UUID, -- gadget_id or swap_id depending on type
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- =============================================================================
-- SWAPS (credit-for-gadget exchanges, local pickup only)
-- =============================================================================
CREATE TABLE swaps (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_swaps_buyer ON swaps(buyer_profile_id);
CREATE INDEX idx_swaps_seller ON swaps(seller_profile_id);
CREATE INDEX idx_swaps_gadget ON swaps(gadget_id);
CREATE INDEX idx_swaps_status ON swaps(status);

-- =============================================================================
-- MESSAGES (in-swap chat between buyer and seller)
-- =============================================================================
...
-- =============================================================================
-- VALUATIONS (optional: store Instant Valuation history for analytics)
-- =============================================================================
CREATE TABLE valuations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  gadget_id UUID REFERENCES gadgets(id) ON DELETE SET NULL,
  -- If pre-listing: no gadget_id yet
  brand TEXT,
  category TEXT,
  model_name TEXT,
  condition TEXT,
...
CREATE INDEX idx_valuations_profile ON valuations(profile_id);
...
CREATE TRIGGER gadgets_updated_at
...
-- Gadgets: public read for available, owners manage
...
-- Swaps: participants can view
CREATE POLICY "Swap participants can view swap" ON swaps FOR SELECT USING (
...
CREATE POLICY "Swap participants can update swap status" ON swaps FOR UPDATE USING (
  buyer_profile_id = auth.uid() OR seller_profile_id = auth.uid()
);

-- Messages: swap participants only
